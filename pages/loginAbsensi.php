...
<body>
 ...
 <div id="id01" class="modal">
                ...
                <form class="modal-content animate" name="form_login" method="post" action="loginAbsensi.php" role="form" width='100%'>
...
<div class="grid-container">
        ...
        <div class="hero-holder grid-100 mobile-grid-100 tablet-grid-100 ut-hero-style-1" style="opacity: 1;">
            <div class="hero-inner" style="text-align:center;">                
                                    <div class="hth"><h1 class="hero-title ut-glow"><span><font color="#FFFFFF">Absensi Euy !</font></span></h1></div>
...
$PWD = md5($PASS);
//$sql= "SELECT USER_ID, USER_PWD, USER_LEVEL FROM SIAK_USER_PLUS";
$sql= "SELECT USER_ID, USER_PWD, USER_LEVEL FROM SIAK_USER_PLUS WHERE USER_ID = '$USR' AND USER_PWD = '$PWD'";
...
$num_rows=oci_fetch($stmt);
	   if($num_rows>0){
		$sql= "select USER_LEVEL from SIAK_USER_PLUS WHERE USER_ID = '$USR' AND USER_PWD = '$PWD'";
		// $sql= "SELECT USER_ID, USER_PWD, USER_LEVEL FROM SIAK_USER_PLUS";
		$stmt = oci_parse($conn222, $sql);
		oci_execute($stmt);
		$row = oci_fetch_array($stmt);
	if($row['USER_LEVEL'] == '11')
	{
	$sql= "select USER_ID from SIAK_USER_PLUS WHERE USER_ID = '$USR' AND USER_PWD = '$PWD'";
	$stmt = oci_parse($conn222, $sql);
	oci_execute($stmt);
	while (($row = oci_fetch_array($stmt, OCI_ASSOC)) != false) {
	header("location: Absensi/index.php?user=".htmlentities(base64_encode($row['USER_ID']))."");
	}
	}
	else
	{
	...
	oci_execute($stmt);
	while (($row = oci_fetch_array($stmt, OCI_ASSOC)) != false) {
	//header("location: Absensi/admin.php?user=$row['USER_ID'])");
		header("location: op/index.php?user=".htmlentities(base64_encode($row['USER_ID']))."");
	}
	...
	else
	{
		...
		// echo "<h1 class='hero-title ut-glow'><font color='red'>$USR Atau $PWD Salah</font></h1>";

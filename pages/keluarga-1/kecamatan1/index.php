        ...
        <div id="page-wrapper">
            <div class="row">
				...
				Include('../config/connect.php');
					...
					$sql= "SELECT NAMA_KEC FROM SETUP_KEC WHERE NO_PROP='32' AND NO_KAB ='73' AND NO_KEC ='1'";
                        ...
                        <div class="panel-body">
                            <table width='100%' class='table table-striped table-bordered table-hover' id='dataTables-example'>
                                ...
                                <tbody>
									<?PHP
									Include('../config/connect.php');
									$sql= "SELECT SETUP_KEL.NO_KEC AS P6, SETUP_KEL.NO_KEL AS P1, SETUP_KEL.NAMA_KEL AS P2, LK AS P3, LP AS P4, LK+LP AS P5 FROM T5_KEPKEL_KELAMIN INNER JOIN SETUP_KEL ON SETUP_KEL.NO_KEL = T5_KEPKEL_KELAMIN.NO_KEL AND SETUP_KEL.NO_KEC = T5_KEPKEL_KELAMIN.NO_KEC AND SETUP_KEL.NO_KAB = T5_KEPKEL_KELAMIN.NO_KAB AND SETUP_KEL.NO_PROP = T5_KEPKEL_KELAMIN.NO_PROP WHERE T5_KEPKEL_KELAMIN.NO_PROP ='32' AND T5_KEPKEL_KELAMIN.NO_KAB ='73' AND T5_KEPKEL_KELAMIN.NO_KEC ='1' AND T5_KEPKEL_KELAMIN.BLN  >= TO_DATE('".$BULAN."','MM/yyyy') AND T5_KEPKEL_KELAMIN.BLN < TO_DATE('".$BULAN."','MM/yyyy')+1";
									$stmt = oci_parse($conn, $sql);
									oci_execute($stmt);
									...
									echo "<td class='center'><a href='../../keluarga-1/kecamatan".htmlentities($row['P6'])."/kelurahan".htmlentities($row['P1'])."/index.php'><i class='fa fa-paw  fa-fw'></i> Detail</a></td>";
                                    echo "</tr>";
									}
									/*
									$sql= "SELECT  sum(LK) AS P3, sum(LP) AS P4, sum(LK)+SUM(LP) AS P5 FROM T5_KEPKEL_KELAMIN INNER JOIN SETUP_KEL ON SETUP_KEL.NO_KEL = T5_KEPKEL_KELAMIN.NO_KEL AND SETUP_KEL.NO_KEC = T5_KEPKEL_KELAMIN.NO_KEC AND SETUP_KEL.NO_KAB = T5_KEPKEL_KELAMIN.NO_KAB AND SETUP_KEL.NO_PROP = T5_KEPKEL_KELAMIN.NO_PROP WHERE T5_KEPKEL_KELAMIN.NO_PROP ='32' AND T5_KEPKEL_KELAMIN.NO_KAB ='73' AND T5_KEPKEL_KELAMIN.NO_KEC ='1'  AND T5_KEPKEL_KELAMIN.BLN  >= TO_DATE('".$BULAN."','MM/yyyy') AND T5_KEPKEL_KELAMIN.BLN < TO_DATE('".$BULAN."','MM/yyyy')+1 ";
									$stmt = oci_parse($conn, $sql);
									oci_execute($stmt);
									$row = oci_fetch_array($stmt);
									echo "<tr class='gradeA'>";
                                    ...
                                    echo "<td class='center'>".htmlentities($row['P4'])."</td>";
                                    echo "<td class='center'>".htmlentities($row['P5'])."</td>";
									echo "<td class='center'><a href='../../tbdisduk.php' class='btn btn-primary btn-xs btn-block'>Kembali</a></td>";
									*/
									?>
								</tbody>
								</thead>

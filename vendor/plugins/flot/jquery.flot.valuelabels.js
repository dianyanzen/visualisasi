    ...
    function init(plot) {
        plot.hooks.draw.push(function (plot, ctx) {
			...
			$.each(plot.getData(), function(ii, series){
				series.seriesIndex = ii; // Workaround, since Flot doesn't set this value anymore
				if (showAsHtml)
				...
				for (var i = 0; i < series.data.length; ++i) {
					if (series.data[i] == null || (showLastValue && i != series.data.length-1))
						continue;
					...
					val = ""+val;
					{
						...
						if (Math.abs(yy-last_y)>20 || last_x<xx){
							...
							if (!showAsHtml) {
								...
								//x_pos = xx;
								x_pos = xx-7*(val.length-1);
								y_pos = yy+6;

								// If the value is on the top of the canvas, we need to push it down a little
								if (yy <= 0) y_pos = 18;
								// The same happens with the x axis if (xx >= plot.width()) x_pos = plot.width();
								//console.log(x_pos);
